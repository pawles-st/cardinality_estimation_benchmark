...
impl<B: BuildHasher> GHLLS<B> {
    ...
    pub fn count(&self) -> f64 {
        ...
        let no_free = self.registers.iter().enumerate().filter(|(i, val)| (*val as i32 - gen_gumbel::BIAS) as f32 - gen_gumbel::mantissa_to_float(self.builder.hash_one(i) as u32) < THRESHOLD).count();
